JokerCoin-qt: Qt4 GUI for JokerCoin
===============================
...
Alternatively, install Qt Creator and open the `JokerCoin-qt.pro` file.

An executable named `JokerCoin-qt` will be built.
...
A warning for people using the *static binary* version of JokerCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of JokerCoin is linked against libdb5.3.
...
installed causes JokerCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
