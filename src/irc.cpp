...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("JokerCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #JokerCoinTEST2\r");
            Send(hSocket, "WHO #JokerCoinTEST2\r");
        } else {
            // randomly join #JokerCoin00-#JokerCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #JokerCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #JokerCoin%02d\r", channel_number).c_str());
        }
